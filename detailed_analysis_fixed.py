...
# Example 2: Find weapons category
print("\n2. WEAPONS CATEGORY (Waffe / Jagd):")
...
print("\n3. PARENT-CHILD RELATIONSHIPS (Kopfbedeckung - headwear):")
base_cn = "AUT.AAA.AAC.AAH.ADL.AAB.AAB.AAL"
...
# More hierarchy examples
print("\n4. JEWELRY CATEGORY (Schmuck):")
jewelry = df3[df3['term'].str.contains('schmuck', case=False, na=False)].head(15)
...
# Export detailed examples to file
with open('data/detailed_examples.txt', 'w', encoding='utf-8') as f:
    f.write("=" * 100 + "\n")
    f.write("MUSEUM MAPPER - REAL DATA EXAMPLES\n")
    f.write("=" * 100 + "\n\n")

    f.write("### DIVERSE OBJECT NAME EXAMPLES (25) ###\n\n")
    for idx, row in df1.head(25).iterrows():
        f.write(f"{idx+1}. {row['ObjectName']:<35} (freq: {row['AnzahlvonObjectName']})\n")

    f.write("\n\n### REFERENCE MAPPING EXAMPLES (20) ###\n\n")
    for original, cleaned, freq in mappings_with_cleanup[:20]:
        f.write(f"{original:<35} -> {cleaned:<25} (freq: {freq})\n")

    f.write("\n\n### THESAURUS HIERARCHY EXAMPLES (20) ###\n\n")
    for idx, row in df3.head(20).iterrows():
        f.write(f"CN: {row['CN']:<45} Term: {row['term']:<30} ID: {row['TermID']}\n")

print("\n\nDetailed examples exported to: data/detailed_examples.txt")
